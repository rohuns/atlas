...
class ConvEncoder(NeuralNetwork):
  ...
  def build_graph(self, input):
    with tf.variable_scope(self.scope_name):
      ...
      conv2 = self.conv2d_relu(drop1, filter_shape=[3, 3, 8, 16], scope_name="conv2")  # (116, 98, 16)
...
class DeconvDecoder(NeuralNetwork):
  ...
  def build_graph(self, input):
    with tf.variable_scope(self.scope_name):
      ...
      deconv1 = self.deconv2d(up1, filter_shape=[3, 3], num_outputs=8, scope_name="deconv1")  # (116, 98, 8)
