...
class ConvEncoder(NeuralNetwork):
  ...
  def build_graph(self, input):
    with tf.variable_scope(self.scope_name):
      conv1 = self.conv2d_relu(input, filter_shape=[5, 5, 1, 8], scope_name="conv1")  # (232, 196, 8)
...
class DeconvDecoder(NeuralNetwork):
  ...
  def build_graph(self, input):
    with tf.variable_scope(self.scope_name):
      ...
      deconv2 = self.deconv2d(up2, filter_shape=[5, 5], num_outputs=1, scope_name="deconv2")  # (232, 196, 1)
